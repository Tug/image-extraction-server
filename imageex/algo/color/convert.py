...
def convert(matrix, TTT, axis=None):
    ...
    if (axis != 0):
        TTT = swapaxes(TTT, 0, axis)
    oldshape = TTT.shape        
    TTT = reshape(TTT, (3, -1))
    OUT = dot(matrix, TTT)
    OUT.shape = oldshape
    if (axis != 0):
        OUT = swapaxes(OUT, axis, 0)
    return OUT
...
def join_colors(c1, c2, c3, axis):
    ...
    c2.shape = newshape
    c3.shape = newshape
    return concatenate((c1, c2, c3), axis=axis)
...
def xyz2lab(xyz, axis=None, wp=whitepoints['D65'][-1], doclip=True):
    ...
    if doclip:
        L = clip(L, 0.0, 100.0)
        a = clip(a, -500.0, 500.0)
        b = clip(b, -200.0, 200.0)
...
def ycbcr_8bit(ycbcr, axis=None):
    ...
    Y = asarray((y * 219 + 16), uint8)
    Cb = asarray((cb * 224 + 128), uint8)
    Cr = asarray((cr * 224 + 128), uint8)
